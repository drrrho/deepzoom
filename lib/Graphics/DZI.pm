...
has 'image'    => (isa => 'Image::Magick', is => 'rw');
...
has 'width'    => (isa => 'Int', is => 'rw', default => 1);
has 'height'   => (isa => 'Int', is => 'rw', default => 1);

sub BUILD {
    my $self = shift;

    if ($self->image) { # defined?
...
sub iterate {
    ...
    $self->image ($image) if $image;  # tuck it away
...
sub descriptor {
    ...
    my ($width, $height) = ($self->width, $self->height ); #dimensions ('total');
...
Copyright 2010, 2014 Robert Barta, all rights reserved.
